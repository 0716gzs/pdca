<template>
  <div class="register-wrap">
    <el-form class="login-container">
      <h3 class="title" style="text-align: center">注册</h3>
      <a href="#" @click="Login" class="existing_account">已有账号?去登录</a>

      <!--      <el-form-item-->
      <!--          label="邮箱"-->
      <!--          prop="email"-->
      <!--      >-->
      <!--        <el-input type="email" v-model="RegisterForm.email" placeholder="用户邮箱" autocomplete="off">-->
      <!--          <el-button type="primary" slot="append" @click="SenEmail" :disabled="sms">{{mes}}</el-button>-->
      <!--        </el-input>-->
      <!--      </el-form-item>-->

      <!--      <el-form-item label="验证码" prop="code" v-show="ShowCode">-->
      ...
      <!--        <el-input type="password" v-model="RegisterForm.password" placeholder="用户密码" autocomplete="off"-->
      <!--                  :disabled="pwd_flag"></el-input>-->
      <!--      </el-form-item>-->
      ...
      <el-form-item
          ...
          prop="username"
      >
        <el-input type="username" v-model="RegisterForm.username" placeholder="用户名" autocomplete="off">
        </el-input>
      </el-form-item>


      <el-form-item label="密码" prop="password">
        <el-input type="password" v-model="RegisterForm.password" placeholder="用户密码" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button
            type="primary"
...
export default {
  ...
  methods: {
    ...
    async Register() {
      let data = this.RegisterForm
      try {
        const res = await this.$api.login.register(data)
        this.$router.push({'path': '/success_response', query: {'account_number': res.data.account_number}})
      } catch (error) {
        console.log(error)
      }
    }
