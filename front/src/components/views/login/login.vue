  ...
  <div class="login-wrap">
    <el-form class="login-container">
      ...
      <el-form-item label="用户名" prop="account_number">
        <el-input type="text" v-model="loginForm.account_number" placeholder="用户账号" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="密码" prop="password">
        <el-input type="password" v-model="loginForm.password" placeholder="用户密码" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item>
        <el-button
            type="primary"
      ...
      <el-row style="text-align: center;margin-top: -10px;">
        ...
        &emsp;
        <el-link type="primary" @click="forgot_password">忘记密码</el-link>
      </el-row>
    </el-form>
  </div>
</template>
...
import {encrypt, decrypt} from "@/utils/utls";
  ...
  methods: {
    async doLogin() {
      ...
      try {
        ...
        SetStorage('UserToken', res.data.token)

        try {
          const res = await this.$api.login.info()
          const encryptData = encrypt(JSON.stringify(res.data))
          SetStorage('UserDate', encryptData)

          const resource_list = []
          for (var i = 0; i < res['data']['resources_list'].length; i++) {
            resource_list.push(res['data']['resources_list'][i]['name'])
            if (res['data']['resources_list'][i]['children']) {
              for (var j = 0; j < res['data']['resources_list'][i]['children'].length; j++) {
                resource_list.push(res['data']['resources_list'][i]['children'][j]['name'])
              }
            }
          }
          const resource_encrypt = encrypt(JSON.stringify(resource_list))
          SetStorage('ResourceList', resource_encrypt)
