...
class JWTUtil:
    ...
    def decode(self, token: str, verify: bool = True) -> settings.JWT_PAYLOAD:
        try:
            d = jwt.decode(token, self.__secret_key, verify=verify)
...
class AppTokenService:
    ...
    @classmethod
    def get_payload(cls, token: str, verify: bool = True):
        return cls.jwt_util.decode(token, verify=verify)
