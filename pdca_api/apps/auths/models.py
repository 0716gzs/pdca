...
class Task(BaseModel):
    title = models.CharField(max_length=200,default=f'未命名的任务{current_date}')  # 任务标题
    category = models.ForeignKey(TaskCategory, on_delete=models.CASCADE, related_name='tasks', blank=True, null=True)  # 分类
    description = models.TextField(blank=True, null=True)  # 任务描述
    due_date = models.DateTimeField(blank=True, null=True)  # 截止日期
    ...
    class Meta:
        db_table = 'task'
        verbose_name = verbose_name_plural = '用户任务'
